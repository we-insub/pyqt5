...
# isChecked() 메서드를 사용할 수 있습니다. 선택 여부에 따라 boolean 값을 반환합니다.
# 일반적인 체크 박스는 선택/해제 상태만을 갖지만, setTristate() 메서드를 사용하면
# '변경 없음(no change)' 상태를 가질 수 있습니다.
# 이 체크 박스는 사용자에게 선택하거나 선택하지 않을 옵션을 줄 때 유용합니다.
# 세 가지 상태를 갖는 체크 박스의 상태를 얻기 위해서는
# checkState() 메서드를 사용합니다. 선택/변경 없음/해제 여부에 따라 각각 2/1/0 값을 반환합니다.
# QButtonGroup 클래스를 사용하면 여러 개의 버튼을 묶어서
# exclusive/non-exclusive 버튼 그룹을 만들 수 있습니다.
...
class MyApp(QWidget):
    ...
    def initUI(self):
        ...
        # 체크박스의 상태가 바뀔 때 발생하는 시그널
        # (stateChanged)을 우리가 정의한 changeTitle() 메서드에 연결합니다.
