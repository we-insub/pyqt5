# https://wikidocs.net/33806

# QLineEdit은 한 줄의 문자열을 입력하고 수정할 수 있도록 하는 위젯입니다.
# echoMode()를 설정함으로써 '쓰기 전용' 영역으로 사용할 수 있습니다.
# 이 기능은 비밀번호와 같은 입력을 받을 때 유용하게 사용됩니다.
# setEchoMode() 메서드로 이러한 모드를 설정할 수 있으며,
# 입력값과 기능은 아래 표와 같습니다.
# Normal 모드를 가장 흔하게 사용하며, 기본 설정값이기도 합니다.
# (예시: setEchoMode(QLineEdit.Normal) 또는 setEchoMode(0))
...
# cut(), copy(), paste() 메서드를 통해 잘라내기, 복사하기, 붙여넣기
# 등의 동작을 수행할 수 있습니다. 또한 setAlignment() 메서드로 텍스트의 정렬을 설정할 수 있습니다.
# 텍스트가 변경되거나 커서가 움직일 때, textChanged(),
# cursorPositionChanged()와 같은 시그널이 발생합니다.
